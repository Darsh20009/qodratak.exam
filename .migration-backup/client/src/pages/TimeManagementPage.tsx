...
const CalendarView: React.FC<{ tasks: Task[] }> = ({ tasks }) => {
  ...
  const priorityColors: Record<string, string> = { high: 'bg-red-500', medium: 'bg-amber-500', low: 'bg-green-500' };
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-start rounded-xl p-1.5 min-h-[44px] transition-all text-sm font-medium border ${
                    isSelected
                      ? 'bg-teal-100 text-white border-teal-400 shadow-lg'
                      : isToday
                      ? 'bg-teal-100 dark:bg-teal-100/40 border-teal-400 dark:border-teal-400 text-teal-700 dark:text-teal-700'
                  ...
                  {dayTasks.length > 0 && (
                    <div className="flex gap-0.5 flex-wrap justify-center mt-0.5">
                      {dayTasks.slice(0, 3).map((t, ti) => (
                        <span key={ti} className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white/80' : priorityColors[t.priority] || 'bg-gray-400'}`} />
                      ))}
                    </div>
                  )}
                </button>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              مهام يوم {selectedDay} {MONTHS_AR[month]}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {selectedTasks.length === 0 ? (
              <div className="text-center py-6 text-gray-400">
                ...
                <p className="text-sm">لا توجد مهام في هذا اليوم</p>
              </div>
            ) : (
              <div className="space-y-2">
                {selectedTasks.map(task => (
                  <div key={task.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-50 dark:bg-gray-800/60">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${priorityColors[task.priority]}`} />
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-medium truncate ${task.completed ? 'line-through text-gray-400' : 'text-gray-800 dark:text-gray-200'}`}>{task.title}</p>
                      {task.category && <p className="text-xs text-gray-500">{task.category}</p>}
                    </div>
                    {task.completed && <CheckCircle className="h-4 w-4 text-green-500 flex-shrink-0" />}
                  </div>
                ))}
              </div>
            )}
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-green-100 dark:bg-green-100/20 rounded-lg">
                  <h4 className="font-medium text-green-700 dark:text-green-700 mb-2">توازن الحياة</h4>
