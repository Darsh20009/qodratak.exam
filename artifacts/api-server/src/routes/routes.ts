...
import { insertQuestionSchema } from "@workspace/db";
  ...
  app.get('/api/user/devices', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (mongoose.connection.readyState === 1) {
        ...
        return res.json({
          devices: publicDevices(mongoUser?.devices, currentDeviceKey),
      ...
      return res.json({
        devices: publicDevices(user?.devices, currentDeviceKey),
        limit: MAX_REGISTERED_DEVICES,
      });
    } catch (error) {
