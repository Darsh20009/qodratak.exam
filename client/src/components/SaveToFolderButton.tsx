...
// استدعاء مكون LoadingSpinner البديل
import {
  Select,
  SelectContent,
  SelectItem,
...
export function SaveToFolderButton({
  ...
  const { data: folders = [], isLoading: isFoldersLoading } = useQuery<any[]>({
  ...
  const createFolderMutation = useMutation({
    ...
    onSuccess: (newFolder) => {
      ...
      form.reset();
      setSelectedFolderId(newFolder._id.toString());
    },
    onError: (error) => {
      toast({
        title: "حدث خطأ",
  ...
  const addToFolderMutation = useMutation({
    mutationFn: async ({ folderId, questionId }: { folderId: string, questionId: number }) => {
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['/api/folders', variables.folderId, 'questions'] });
      toast({
