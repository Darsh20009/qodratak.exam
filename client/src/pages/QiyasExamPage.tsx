...
const qiyasExams: QiyasExam[] = [
  {
    ...
    description: "اختبار تأهيلي شامل يتكون من سبعة أقسام متنوعة بين اللفظي والكمي. يتضمن 20 سؤالاً تجريبياً غير محسوب للمساعدة. متاح مجاناً كل يومين.",
    totalSections: 7,
    totalQuestions: 120,
    totalTime: 120,
    isQiyas: true,
                ...
                {exam.requiresSubscription && (
                  <div className={cn(
                    ...
                    {isUserSubscribed ? (
                      ...
                      <> <LockIcon size={16} /> {exam.id === 1 ? 'متاح مجاناً كل يومين' : 'يتطلب اشتراك مدفوع'} </>
                    )}
                  </div>
                )}
              </CardContent>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .options-list li.selected { 
            ...
            background-color: #e9eefe; /* Light indigo */
        ...
        @media (max-width: 768px) {
            ...
            .option-letter { min-width: 35px; height: 35px; font-size: 0.95rem; margin-left: 12px; }
            button { padding: 12px 22px; font-size: 1rem; margin: 5px; min-width: 110px; }
                ...
                <Button
                  onClick={() => {
                    try {
                      ...
                      const wrongQuestions: Array<{question: any, sectionName: string, questionIndex: number}> = [];
                      Object.entries(allProcessedQuestionsBySection).forEach(([sectionNum, sectionQuestions]) => {
                        ...
                        sectionQuestions.forEach((question, index) => {
                          ...
                          if (!isCorrect && answers[question.id] !== undefined && !question._isNonScored) {
                            wrongQuestions.push({
                              question,
                              sectionName,
                      ...
                      if (wrongQuestions.length === 0) {
                        toast({ title: "لا توجد أخطاء!", description: "لم تجب خطأ على أي سؤال. أحسنت!", variant: "default" });
                        return;
                      }
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>تحدي الأخطاء - ${examName} - قدراتك</title>
    ...
    <style>
        * {
        ...
        body {
            ...
            background: linear-gradient(135deg, #0f0f23 0%, #1a1a3a 25%, #2d1b4e 50%, #3d1a5c 75%, #4a1b6b 100%);
            min-height: 100vh;
            color: #e2e8f0;
            position: relative;
            overflow-x: hidden;
        ...
        .logo {
            font-size: 4.5rem;
            font-weight: 900;
            background: linear-gradient(135deg, #8b5cf6, #ec4899, #f59e0b, #ef4444);
        ...
        @keyframes glow {
            ...
            to { text-shadow: 0 0 30px rgba(139, 92, 246, 0.8), 0 0 40px rgba(236, 72, 153, 0.5); }
        }
    </style>
</head>
<body>
    ...
    <div class="container">
        <div class="header">
            <div class="logo glow">قدراتك</div>
            <h1 class="title">🎯 تحدي الأخطاء الفخم</h1>
            <p class="subtitle">تعلم من أخطائك واصبح أقوى</p>
            <p class="subtitle">${examName}</p>
            <p style="font-size: 1.1rem; opacity: 0.8; color: #c084fc;">تم الإنشاء في: ${currentDate}</p>
        </div>
        
        <div class="challenge-stats">
            <div class="stat-card">
                <div class="stat-value">${wrongQuestions.length}</div>
                <div class="stat-label">أخطاء للمراجعة</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">${Object.keys(allProcessedQuestionsBySection).length}</div>
                <div class="stat-label">أقسام الاختبار</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">100%</div>
                <div class="stat-label">فرصة التحسن</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">⭐</div>
                <div class="stat-label">التحدي الذهبي</div>
            </div>
        </div>
        
        <div class="challenges-section">
            <h2 class="section-title">🔥 أخطاؤك تحديك</h2>
            ...
            ${wrongQuestions.map((item, index) => `
                <div class="question-card">
                    ...
                    <div class="question-info">
                        <div class="section-badge">${item.sectionName}</div>
                        <span style="color: #94a3b8; font-weight: 600;">السؤال رقم ${item.questionIndex}</span>
                    </div>
                    ...
                    <div class="options-grid">
                        ${item.question.options.map((option: string, optIndex: number) => `
                            <div class="option ${
                                optIndex === answers[item.question.id] ? 'user-wrong' : 
                                optIndex === item.question.correctOptionIndex ? 'correct' : ''
                            }">
                                ${String.fromCharCode(65 + optIndex)}. ${option}
                                ${optIndex === answers[item.question.id] ? ' <span style="font-size: 0.9rem; opacity: 0.8;">(إجابتك الخاطئة)</span>' : ''}
                                ${optIndex === item.question.correctOptionIndex ? ' <span style="font-size: 0.9rem; opacity: 0.8;">(الإجابة الصحيحة)</span>' : ''}
                            </div>
                        `).join('')}
                    </div>
                    ...
                    ${item.question.explanation ? `
                        <div class="explanation">
                            <div class="explanation-title">الشرح والتوضيح المفصل:</div>
                            <div class="explanation-text">${item.question.explanation}</div>
                        </div>
                    ` : `
                        <div class="explanation">
                            <div class="explanation-title">ملاحظة مهمة:</div>
                            <div class="explanation-text">
                                إجابتك كانت خاطئة. الإجابة الصحيحة هي الخيار ${String.fromCharCode(65 + item.question.correctOptionIndex)}.
                                راجع هذا النوع من الأسئلة وتدرب عليه أكثر لتجنب الأخطاء المشابهة.
                            </div>
                        </div>
                    `}
                </div>
        ...
        <div class="challenge-tips">
            <h3 class="tips-title">💎 نصائح ذهبية للتحسن</h3>
            <ul class="tips-list">
                <li>راجع كل خطأ بعناية وحاول فهم السبب وراء الإجابة الصحيحة</li>
                <li>تدرب على نفس نوع الأسئلة التي أخطأت فيها</li>
                <li>اقرأ الشروحات بتمعن لفهم الطريقة الصحيحة للتفكير</li>
                <li>حاول حل هذه الأسئلة مرة أخرى بعد فترة لترى تحسنك</li>
                <li>لا تستسلم - كل خطأ هو خطوة نحو الإتقان</li>
            </ul>
        </div>
        
        <div class="footer">
            <div class="footer-text">
                تم إنشاء هذا التحدي بواسطة منصة <span class="gradient-text">قدراتك</span> 
                <br>
                منصة شاملة لتطوير القدرات والاستعداد للاختبارات المعيارية
                <br>
                <strong style="color: #8b5cf6;">التعليم حق للجميع</strong>
            </div>
        </div>
    </div>
</body>
                      ...
                      link.href = url;
                      link.download = `تحدي_الاخطاء_${examName.replace(/\s+/g, '_')}_${new Date().getTime()}.html`;
                      ...
                      toast({ 
                        title: "تم تحميل تحدي الأخطاء!", 
                        description: `ملف فخم يحتوي على ${wrongQuestions.length} سؤال خاطئ للمراجعة.`, 
                        duration: 5000 
                      });
                    } catch (error) {
                      console.error('خطأ في تحميل تحدي الأخطاء:', error);
                      toast({ title: "خطأ", description: "حدث خطأ أثناء إنشاء ملف التحدي.", variant: "destructive" });
                    }
                  }}
                  variant="default"
                  className="gap-2 w-full sm:w-auto bg-gradient-to-r from-purple-600 via-red-600 to-pink-600 hover:from-purple-700 hover:via-red-700 hover:to-pink-700 text-white shadow-lg hover:shadow-xl transition-all duration-300"
                >
                  <Target className="h-4 w-4" />
                  {isMobile ? 'تحميل تحدي الأخطاء الفخم' : 'تحدي الأخطاء الفخم'}
                </Button>
                <Button
                  onClick={() => {
                    try {
